...
def _demo():
    ...
    arg_parser.add_argument("--save-encoded", help="Path to save encoded features (CSV file)")
    ...
    source = rf.convert_to_tensor(data_pt, dims=[batch_dim, audio_spatial_dim])

    rf.set_default_device(dev_s)

    # Run model inference
    logits, enc, enc_spatial_dim = model(source, in_spatial_dim=audio_spatial_dim)

    # Process for CTC decoding
    ...
    try:
        labels_raw = labels_raw[0, : labels_seq_lens[0]]
        labels_s = model.target_dim.vocab.get_seq_labels(labels_raw)
        prediction_text = labels_s.replace(" ", "").replace("▁", " ").strip()
    except Exception as e:
        print("Warning: could not decode labels:", e)
        labels_s = ""
        prediction_text = "UNKNOWN"

    print("Recognized label sequence:", labels_s)
    print("SPM-to-words:", prediction_text)

    # Save encoded features as CSV with predicted label as comment
    if args.save_encoded:
        encoded_numpy = enc.raw_tensor.detach().cpu().numpy()

        csv_path = args.save_encoded
        if csv_path.endswith(".npy"):
            csv_path = csv_path.replace(".npy", ".csv")
        elif not csv_path.endswith(".csv"):
            csv_path = csv_path + ".csv"

        csv_comment = f"# Recognized label: {prediction_text}"
        np.savetxt(
            csv_path,
            encoded_numpy.reshape(-1, encoded_numpy.shape[-1]),
            delimiter=",",
            header=csv_comment,
            comments=""
        )
        print(f"Encoded features and label saved to CSV: {csv_path}")
...
def create_model(*, load_params: bool = True, device: Optional[torch.device] = None) -> Model:
    ...
    if load_params:
        ...
        checkpoint_state = torch.load(filename, map_location=device, weights_only=False)
