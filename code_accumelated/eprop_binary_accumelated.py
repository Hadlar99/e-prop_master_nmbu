...
## Schematic for network architecture
try:
...
n_batch = 3  # batch size, 64 in reference [2], 32 in the README to reference [2]
...
def generate_evidence_accumulation_input_output(
    n_batch, n_in, input_data_list, n_cues, n_input_symbols, steps
):
    n_pop_nrn = n_in // n_input_symbols
    
    input_spike_probs = np.zeros((n_batch, steps["sequence"], n_in))
    target_cues = np.zeros(n_batch, dtype=int)  # Store targets per batch
    chosen_datasets = np.zeros(n_batch, dtype=int)  # Store dataset index per batch

    for b_idx in range(n_batch):
        # Randomly pick one dataset from the list for this batch
        chosen_index = np.random.randint(len(input_data_list))
        chosen_datasets[b_idx] = chosen_index  # Store which dataset was used
        
        # Ensure we pick the correct cues for this batch
        batched_cues = np.array(input_data_list[chosen_index][b_idx], dtype=int)

        for c_idx in range(n_cues):
            cue = batched_cues[c_idx]  # Pick cue correctly
            ...
            # Directly use the data to set spike probabilities
            input_spike_probs[b_idx, step_start:step_stop, pop_nrn_start:pop_nrn_stop] = input_data_list[chosen_index][b_idx][c_idx]

        # Recall activation should end properly after recall duration
        recall_start = steps["sequence"] - steps["recall"]
        recall_stop = steps["sequence"]
        input_spike_probs[b_idx, recall_start:recall_stop, n_pop_nrn:2 * n_pop_nrn] = 0.3  # Full activation during recall

        # Store target cue per batch
        target_cues[b_idx] = chosen_index  # Store which dataset was used as the target

    input_spike_bools = input_spike_probs > np.random.rand(input_spike_probs.size).reshape(input_spike_probs.shape)
    input_spike_bools[:, 0, :] = 0  # Remove spikes in 0th time step

    return input_spike_bools, target_cues, chosen_datasets


# Call the function as usual, without input_spike_prob
spike_bools, targets, chosen_datasets = generate_evidence_accumulation_input_output(
    n_batch, n_in, data, n_cues, n_input_symbols, steps
)

print("Chosen datasets per batch & iteration:")
...
target_cues_list = []
chosen_datasets_list = []  # Store chosen datasets

for iteration in range(n_iter):
    input_spike_bools, target_cues, chosen_datasets = generate_evidence_accumulation_input_output(
        n_batch, n_in, data, n_cues, n_input_symbols, steps
    )
    
    input_spike_bools_list.append(input_spike_bools)
    target_cues_list.extend(target_cues.tolist())
    chosen_datasets_list.extend(chosen_datasets.tolist())  # Keep track of chosen datasets

# Reshape input spike array
...
# Generate target rate parameters
...
for xlims in [(0, steps["sequence"]), (steps["task"] - steps["sequence"], steps["task"])]:
    ...
    fig.align_ylabels()
    plt.savefig("spikes_and_dynamic_variables_{}.png".format(xlims[0]))
...
fig.align_ylabels()
fig.tight_layout()
plt.savefig("weight_time_courses.png")
...
plt.savefig("weight_matrices.png")
plt.show()
