...
output_dir = f"results_val_{today}_45_240_4_8_list_1"
...
# Set learning rate to 0 so no learning happens during validation
params_common_syn_eprop = {
    "optimizer": {
        "type": "adam",
        "batch_size": n_batch,  # Use the same batch size as in training
        "beta_1": 0.9,
        "beta_2": 0.999,
        "epsilon": 1e-8,
        "eta": 0.0,  # <--- no learning!
    ...
    # weight_recorder optional in validation
}

# Apply common settings to all e-prop synapses
...
# Feedback, normalization, and target rate connections
params_syn_feedback = {
    "synapse_model": "eprop_learning_signal_connection_bsshslm_2020",
    "delay": duration["step"],
    "weight": 1.0,  # Placeholder, weights will be overwritten
}
...
def apply_loaded_weights(csv_path):
    ...
    for _, row in df.iterrows():
        ...
        try:
            conn = nest.GetConnections(
                nest.NodeCollection([source_id]),
                nest.NodeCollection([target_id])
            )
            if len(conn):
...
apply_loaded_weights("/mnt/users/hastabbe/e-prop_master_nmbu/code_accumelated/orion_scripts_train_4_8_val/results_2025-05-01_240_neruons_4_8_list_1/weights_240.csv")
...
train_number = list_train["New List"].tolist()
...
# F1-score per training iteration (macro average)
f1_scores_macro = []
for i in range(n_iter):
    y_pred_iter = y_prediction[i]
    y_true_iter = y_target[i]
    f1_iter = f1_score(y_true_iter, y_pred_iter, average='macro', zero_division=0)
    f1_scores_macro.append(f1_iter)
...
ax.plot(range(1, n_iter + 1), f1_scores_macro)
