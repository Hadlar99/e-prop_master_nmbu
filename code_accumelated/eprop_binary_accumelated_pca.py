...
n_batch = 4  # batch size, 64 in reference [2], 32 in the README to reference [2]
...
# Combine the data
data_comb = pd.concat(data)

# do a PCA on the data
pca = PCA(n_components=15)
data_pca = pca.fit_transform(data_comb)

scaler = MinMaxScaler(feature_range=(0, 500))
data_pca = scaler.fit_transform(data_pca)

data_1_pca = pca.transform(data_1)
data_2_pca = pca.transform(data_2)

# normalize the data
data_1_pca = scaler.transform(data_1_pca)
data_2_pca = scaler.transform(data_2_pca)


data_pca = [data_1_pca, data_2_pca]
...
for _ in range(n_iter):
    ...
    n_batch, n_in, data_pca, n_cues, steps
)
